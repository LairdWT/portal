...
- web @ plugins/web/CONVENTIONS.md (v0.29.0)

Project sections below override the referenced conventions where they
intersect. Drift gate: `agent-code-skills pins --check .`.

## Verification

- Full gate: `pnpm verify` (typecheck, lint, css lint, format check,
  unit + story tests, build).
- Noisy output (vitest, playwright, vite): route through
  `agent-code-skills logstrip run -- <cmd>`.
- ADR gate: `python docs/adr/check-adrs.py` (0 clean, 1 findings).
